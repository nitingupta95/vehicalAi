...
import { Calendar, Shield, ChevronDownIcon  } from "lucide-react";
...
export default async function Home() {
  ...
  return (
    <div className="pt-20 flex flex-col">
      {/* Hero */}
      <section className="relative py-16 md:py-28 dotted-background px-12">
    ...
    <section className="py-16 text-black">
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <Accordion type="single" collapsible className="w-full divide-y divide-white/10 border border-white/10 rounded-lg overflow-hidden">
          {faqItems.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger className="group flex justify-between items-center w-full py-5 px-4 text-left font-medium hover:text-black transition-colors duration-300">
                {faq.question}
                <ChevronDownIcon className="h-5 w-5 transition-transform duration-300 group-data-[state=open]:rotate-180" />
              </AccordionTrigger>

              <AccordionContent className="px-4 pb-5 text-gray-600 text-sm leading-relaxed animate-slide-down">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
